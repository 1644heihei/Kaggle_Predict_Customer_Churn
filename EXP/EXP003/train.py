...
LightGBMとXGBoostの比較によるアンサンブル用多様性の確保。
EXP002と同じ特徴量を使用し、モデルのみXGBoostに変更。
...
def train_cv(X_train, y_train, config):
    ...
    # 特徴重要度を統合
    importance_dict = {}
    for fold_importance in feature_importance_list:
        for feature, score in fold_importance.items():
    ...
    importance_df["importance"] = importance_df["importance"] / len(models)
...
def main():
    ...
    # テスト予測
    test_predictions = inference(X_test, models)
